...
The **Multi-Dialect SQL PR Reviewer Agent** is an automated code review system designed to enforce corporate **SQL Naming & Coding Standards** (defined in the PDF specification) across database pull requests and data pipeline repositories (PostgreSQL, Snowflake, BigQuery, MySQL, etc.).
...
- ❌ **`Amt`** $\rightarrow$ Use `Amount`
- ❌ **`Cnt`** $\rightarrow$ Use `Count`
- ❌ **`Flg`** $\rightarrow$ Use `Is...` / `Has...` / `Can...` boolean prefix
- ❌ **`Desc`** $\rightarrow$ Use `Description`
